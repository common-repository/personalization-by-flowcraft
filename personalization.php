...
include 'includes/Mobile_Detect_FC.php';
...
class PersonalizationPlugin
{
    ...
    public function __construct()
    {
        ...
        foreach ($fields as $key => $value) {
            if (!PersCookie::has($key)) {
                PersCookie::set($key, $value);
            }
        }
    ...
    function initActions()
    {
        ...
        if (!is_admin()) {
            PersCookie::set('visits', (int)PersCookie::get('visits') + 1, time()+31556926, '/');
            
            //set cookies only for the pages that are included in user-visits-page conditions
            $pagechecks = $this->getPagesWithConditions();
            if(!empty($pagechecks) && in_array($currentLink, $pagechecks ))
                PersCookie::incrementPageVisit($currentLink);

            $locationData = PersCookie::get('user-location');
            if (empty($locationData) || $locationData == '0') {
                $response = $this->getUserInfoByIp();
                if (!empty($response)) {
                    $locationData = @unserialize($response);
                    PersCookie::set('user-location', $locationData, strtotime("+ 30 minutes", time()));
                }
            }
        }
    }
    ...
    /**
     * Get post with conditions set for them
     *
     * 
     * @return array
     */
     function getPagesWithConditions()
     {
         global $wpdb;
        $pagerules = $wpdb->get_results(  $wpdb->prepare( "
            SELECT pm.meta_value AS pagerule  
            FROM {$wpdb->postmeta} pm
                LEFT JOIN {$wpdb->posts} p ON p.ID = pm.post_id
            WHERE pm.meta_key = '%s'
                AND pm.meta_value != '' 
                AND pm.meta_value LIKE '%user-visits-page%'
                AND p.post_type = '%s'
                AND p.post_status = 'publish'
            
            ORDER BY pm.meta_value          
            ", 'rules', 'personalization-cnd') 
        );
        
        $pagechecks = array();
        $pagerules = json_decode(json_encode($pagerules), true);
        if(!empty($pagerules))
        {
            foreach($pagerules as $pr)
            {
                $prd = json_decode($pr['pagerule'],true);
                $pagechecks[] = $prd[0]['value1'];
            }
        } 
    ...
    function getConditionOutput($postId)
    {
        ...
        $visits = (int)PersCookie::get('visits');
            ...
            foreach ($rules as $key => $rule) {
                ...
                switch ($rule['name']) {
                    case FIRST_TIME_USER:
                        ...
                        break;
                    case USER_IS_FROM:
                        $userLocation = PersCookie::get('user-location');
                    ...
                    case USER_VISITS_PAGE:
                        if ($rule['value2'] == 'every-time') {
                            ...
                            $rule['value2'] == 'exactly'
                            && PersCookie::getPageCount($rule['value1']) == $rule['value3']
                        ) {
                            $statement .= " true";
                        } else if (
                            $rule['value2'] == 'more-than'
                            && (int)PersCookie::getPageCount($rule['value1']) > $rule['value3']
                        ) {
                            $statement .= " true";
                        } else {
                            $statement .= " false";
                    ...
                    case USER_TRIES_TO_LEAVE_SITE:
                        ...
                        $leftSite = PersCookie::get('user-leaves');
                        //die(var_dump($leftSite));
                        if (empty($leftSite)) {
                            $userLeaves[$key] = TRUE;
                        }
                        $statement .= "false";
                    ...
                    case USER_SCROLLS:
                        ...
                        break;
                    case DEVICE_IS:
                        $detect = new Mobile_Detect_FC;
                        if (
                            ($rule['value1'] == 'phone' && $detect->isMobile() && !$detect->isTablet())
            ...
            if (!empty($userScrolls)) {
                ...
                $output .= "var isScrolling;
                        ...
                        if(userLeaveRules.length !== 0){
                           var isLeaving;
                           window.clearTimeout( isLeaving );
                           isLeaving = setTimeout(function() {
                                var userLeft = false;
                                var lastYposition = 0;
                                $('body').mousemove(function(e){
                                    if(e.clientY < lastYposition && e.clientY <= 50 && !userLeft) {
                                        userLeft = true;
                                        let count = 0;
    ...
    {
        PersCookie::set('user-leaves', '1', strtotime("+60 days"));
    }
}
...
class PersCookie
{
    ...
    public static function incrementPageVisit($link)
    {
        ...
        setcookie($cookieName, $value, time()+31556926, '/');
...
$personalizationPlugin = new PersonalizationPlugin();
